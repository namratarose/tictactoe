...
public class TicTacToe extends JPanel {
  ...
  public void createButtons() {
    for (int i = 0; i <= 8; i++) {
      ...
      jButtons[i].addActionListener(
          e -> {
            JButton clickedBtn = (JButton) e.getSource();
            clickedBtn.setText(String.valueOf(playerSign));
            clickedBtn.setFont(new Font("Arial", Font.BOLD, 200));
            clickedBtn.setEnabled(false);

            if (playerSign == 'x') playerSign = 'o'; else playerSign = 'x';
  ...
  public boolean checkAllRows() {
    ...
    for (int j = 0; j < 3; j++) {
      if (
        jButtons[i].getText().equals(jButtons[i + 1].getText()) &&
  ...
  public boolean checkAllColumns() {
    ...
    for (int j = 0; j < 3; j++) {
      if (
        jButtons[i].getText().equals(jButtons[i + 3].getText()) &&
  ...
  public boolean checkTheDiagonals() {
    if (
      jButtons[0].getText().equals(jButtons[4].getText()) &&
      jButtons[0].getText().equals(jButtons[8].getText()) &&
      !jButtons[0].getText().equals("")
    ) return true; else return (
      jButtons[2].getText().equals(jButtons[4].getText()) &&
